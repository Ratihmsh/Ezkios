...
@section('page-subtitle', 'Buat aturan diskon, kupon, atau kenaikan harga')
        ...
        <form action="{{ route('promotions.store') }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" required placeholder="Contoh: Diskon Kemerdekaan, Promo BCA">
                    @error('name')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="type" id="type" class="form-select @error('type') is-invalid @enderror" required>
                        <option value="product_discount" {{ old('type') == 'product_discount' ? 'selected' : '' }}>Diskon Produk Spesifik</option>
                        <option value="product_markup" {{ old('type') == 'product_markup' ? 'selected' : '' }}>Kenaikan Harga (Markup) Produk Spesifik</option>
                        <option value="category_discount" {{ old('type') == 'category_discount' ? 'selected' : '' }}>Diskon per Kategori Produk</option>
                        <option value="transaction_discount" {{ old('type') == 'transaction_discount' ? 'selected' : '' }}>Diskon Global (Transaksi)</option>
                        <option value="buy_x_get_y" {{ old('type') == 'buy_x_get_y' ? 'selected' : '' }}>Buy X Get Y (Beli 1 Gratis 1, dsb)</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3" id="reward_product_container" style="display: none;">
                    ...
                    <select name="reward_product_id" id="reward_product_id" class="form-select @error('reward_product_id') is-invalid @enderror">
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" {{ old('reward_product_id') == $product->id ? 'selected' : '' }}>
            ...
            <div class="row">
                <div class="col-md-4 mb-3" id="min_qty_container">
                    ...
                    <input type="number" name="min_qty" id="min_qty" class="form-control @error('min_qty') is-invalid @enderror" value="{{ old('min_qty', 1) }}" required min="1">
                    <small class="text-muted">Contoh: 2 (Untuk Beli 2)</small>
                    @error('min_qty')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4 mb-3" id="reward_qty_container" style="display: none;">
                    ...
                    <input type="number" name="reward_qty" id="reward_qty" class="form-control @error('reward_qty') is-invalid @enderror" value="{{ old('reward_qty', 1) }}" min="1">
                    <small class="text-muted">Contoh: 1 (Gratis 1)</small>
                    @error('reward_qty')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3" id="min_spend_container" style="display: none;">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" name="min_spend" id="min_spend" class="form-control @error('min_spend') is-invalid @enderror" value="{{ old('min_spend', 0) }}" required min="0">
                    </div>
                    @error('min_spend')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row" id="value_container">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="value_type" id="value_type" class="form-select @error('value_type') is-invalid @enderror" required>
                        ...
                        <option value="fixed_amount" {{ old('value_type') == 'fixed_amount' ? 'selected' : '' }}>Nominal (Rupiah)</option>
                    </select>
                    @error('value_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" step="0.01" name="value" id="value" class="form-control @error('value') is-invalid @enderror" value="{{ old('value', 0) }}" required>
                    @error('value')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="text" name="promo_code" id="promo_code" class="form-control @error('promo_code') is-invalid @enderror" value="{{ old('promo_code') }}" placeholder="Contoh: MERDEKA2026">
                    <small class="text-muted">Jika diisi, promo hanya berlaku bila kode dimasukkan.</small>
                    @error('promo_code')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <small class="text-muted">Maksimal promo ini dapat digunakan. Kosongkan = unlimited.</small>
                    @error('usage_limit')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleFields() {
            ...
            document.getElementById('min_spend').value = 0;
