...
<div class="card">
    <div class="card-header">
        <i class="bi bi-arrow-down-circle"></i> <strong>Detail Pembelian</strong>
    </div>
    <div class="card-body">
        <div class="row">
            <div class="col-md-6">
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ asset('storage/' . $purchase->receipt_image) }}" target="_blank" class="btn btn-sm btn-outline-info">
                                <i class="bi bi-file-earmark-image"></i> Lihat Nota
                            </a>
                        </td>
                    </tr>
                    @endif
            ...
            <div class="col-md-6">
                <table class="table">
                    ...
                    <tr>
                        <th>Pajak</th>
                        <td>Rp {{ number_format($purchase->tax ?? 0, 0, ',', '.') }}</td>
                    </tr>
                    <tr>
                        <th>Ongkos Kirim</th>
                        <td>Rp {{ number_format($purchase->shipping_cost ?? 0, 0, ',', '.') }}</td>
                    </tr>
                    <tr>
                        <th><strong>Grand Total</strong></th>
                        <td><strong>Rp {{ number_format($purchase->grand_total, 0, ',', '.') }}</strong></td>
                    </tr> --}}
                    @if($purchase->payment_status === 'partial')
                    {{-- <tr>
                        <th>Jumlah Dibayar</th>
                        <td class="text-success">Rp {{ number_format($purchase->paid_amount, 0, ',', '.') }}</td>
                    </tr>
                    <tr>
                        <th>Sisa Tagihan</th>
                        <td class="text-danger">Rp {{ number_format($purchase->grand_total - $purchase->paid_amount, 0, ',', '.') }}</td>
                    </tr> --}}
                    @elseif($purchase->payment_status === 'paid')
                    {{-- <tr>
                        <th>Jumlah Dibayar</th>
                        <td class="text-success">Rp {{ number_format($purchase->grand_total, 0, ',', '.') }}</td>
                    </tr> --}}
                    @endif
                    <tr>
                        <th>Dibuat Oleh</th>
                    ...
                    <table class="table table-striped">
                        ...
                        <tfoot>
                            ...
                            <tr>
                                ...
                                <th>Rp {{ number_format($purchase->tax ?? 0, 0, ',', '.') }}</th>
                            </tr>
                            <tr>
                                <th colspan="4" class="text-end">Ongkos Kirim</th>
                                <th>Rp {{ number_format($purchase->shipping_cost ?? 0, 0, ',', '.') }}</th>
                            </tr>
                            <tr class="table-primary">
                                <th colspan="4" class="text-end"><strong>Grand Total</strong></th>
                                <th><strong>Rp {{ number_format($purchase->grand_total, 0, ',', '.') }}</strong></th>
                            </tr>
                            <tr class="table-success">
                                <th colspan="4" class="text-end"><strong>Sudah Dibayar</strong></th>
                                <th><strong>Rp {{ number_format($purchase->paid_amount, 0, ',', '.') }}</strong></th>
                            </tr>
                            <tr class="table-danger">
                                <th colspan="4" class="text-end"><strong>Sisa Tagihan</strong></th>
                                <th><strong>Rp {{ number_format($purchase->grand_total - $purchase->paid_amount, 0, ',', '.') }}</strong></th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
        ...
        <div class="d-flex justify-content-between mt-3">
            ...
            <div>
                ...
                @endif
                <a href="{{ route('purchases.print', $purchase) }}" class="btn btn-secondary" target="_blank">
...
<div class="modal fade" id="paymentModal" tabindex="-1" aria-labelledby="paymentModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('purchases.payments.store', $purchase) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control fw-bold" readonly value="Rp {{ number_format($purchase->grand_total - $purchase->paid_amount, 0, ',', '.') }}">
                        <input type="hidden" id="raw_remaining_balance_show" value="{{ intval($purchase->grand_total - $purchase->paid_amount) }}">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="due_date" id="due_date" class="form-control" value="{{ $purchase->due_date ? \Carbon\Carbon::parse($purchase->due_date)->format('Y-m-d') : '' }}">
