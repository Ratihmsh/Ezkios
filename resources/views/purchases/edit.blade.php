...
@section('page-subtitle', 'Ubah data pembelian barang masuk')
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('purchases.update', $purchase) }}" method="POST" id="purchaseForm" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="invoice_number" id="invoice_number" class="form-control bg-light @error('invoice_number') is-invalid @enderror" value="{{ old('invoice_number', $purchase->invoice_number) }}" readonly required>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <input type="date" name="purchase_date" id="purchase_date" class="form-control @error('purchase_date') is-invalid @enderror" value="{{ old('purchase_date', $purchase->purchase_date->format('Y-m-d')) }}" required>
                        @error('purchase_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    <div class="mb-2">
                                        ...
                                        <input type="number" name="items[{{ $index }}][quantity]" class="form-control quantity" value="{{ old("items.$index.quantity", $item->quantity) }}" min="1" required autocomplete="off">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="mb-2">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" name="items[{{ $index }}][purchase_price]" class="form-control purchase-price" value="{{ old("items.$index.purchase_price", number_format($item->purchase_price, 0, ',', '.')) }}" required autocomplete="off">
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="mb-2">
                                        ...
                                        <div class="input-group">
                                            <span class="input-group-text">Rp</span>
                                            <input type="text" class="form-control subtotal" value="{{ number_format($item->subtotal, 0, ',', '.') }}" autocomplete="off">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="row mt-2">
                                <div class="col-md-10">
                                    <div class="mb-2">
                                        ...
                                        <input type="text" name="items[{{ $index }}][notes]" class="form-control" value="{{ old("items.$index.notes", $item->notes) }}" placeholder="Catatan untuk item ini" autocomplete="off">
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    @if($loop->count > 1)
            ...
            <div class="row mt-4">
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="text" name="tax" id="tax" class="form-control nominal-input" value="{{ old('tax', number_format($purchase->tax ?? 0, 0, ',', '.')) }}" autocomplete="off">
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="text" name="shipping_cost" id="shipping_cost" class="form-control nominal-input" value="{{ old('shipping_cost', number_format($purchase->shipping_cost ?? 0, 0, ',', '.')) }}" autocomplete="off">
                        </div>
                    </div>
                </div>
                <div class="col-md-4 offset-md-4">
                    <div class="mb-3">
                        ...
                        <input type="text" id="grand_total" class="form-control bg-light text-end text-primary" readonly style="font-weight: bold; font-size: 1.5rem;" value="Rp {{ number_format($purchase->grand_total, 0, ',', '.') }}">
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select name="payment_status" id="payment_status" class="form-control @error('payment_status') is-invalid @enderror" required>
                            <option value="pending" {{ old('payment_status', $purchase->payment_status) == 'pending' ? 'selected' : '' }}>Belum Dibayar</option>
                            <option value="partial" {{ old('payment_status', $purchase->payment_status) == 'partial' ? 'selected' : '' }}>Dibayar Sebagian</option>
                            <option value="paid" {{ old('payment_status', $purchase->payment_status) == 'paid' ? 'selected' : '' }}>Lunas</option>
                        </select>
                ...
                <div class="col-md-4 d-none" id="paid_amount_container">
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="text" name="paid_amount" id="paid_amount" class="form-control nominal-input @error('paid_amount') is-invalid @enderror" value="{{ old('paid_amount', number_format($purchase->paid_amount ?? 0, 0, ',', '.')) }}" autocomplete="off">
                        </div>
                        @error('paid_amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select name="payment_method" id="payment_method" class="form-control @error('payment_method') is-invalid @enderror" required>
                            ...
                            @php
                                $currentMethod = old('payment_method', $purchase->payment_method);
                                $found = false;
                            @endphp
                            @foreach($paymentMethods as $pm)
                                @php if($pm == $currentMethod) $found = true; @endphp
                                <option value="{{ $pm }}" {{ $currentMethod == $pm ? 'selected' : '' }}>{{ $pm }}</option>
                            @endforeach
                            @if($currentMethod && !$found)
                ...
                <div class="col-md-4" id="due_date_container">
                    <div class="mb-3">
                        ...
                        <input type="date" name="due_date" id="due_date" class="form-control" value="{{ old('due_date', $purchase->due_date ? \Carbon\Carbon::parse($purchase->due_date)->format('Y-m-d') : '') }}">
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @if($purchase->receipt_image)
                            <div class="mb-2">
                                <a href="{{ asset('storage/' . $purchase->receipt_image) }}" target="_blank" class="btn btn-sm btn-outline-info">
                                    <i class="bi bi-file-earmark-image"></i> Lihat Nota Saat Ini
                                </a>
                            </div>
                        @endif
                        <input type="file" name="receipt_image" id="receipt_image" class="form-control" accept="image/*">
                        <small class="text-muted">Format: JPG, PNG. Maksimal 5MB. Biarkan kosong jika tidak ingin mengubah nota lama.</small>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="notes" class="form-label">Catatan Tambahan</label>
                        <textarea name="notes" id="notes" rows="2" class="form-control">{{ old('notes', $purchase->notes) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-between mt-4">
                ...
                <button type="submit" class="btn btn-primary btn-lg px-5">
                    <i class="bi bi-save"></i> Update Pembelian
                </button>
            </div>
        </form>
    </div>
        ...
        .then(data => {
            if(data.success) {
                ...
                if(activeProductSelect) {
                    ...
                    updateProfit(row, parseRibuan(row.querySelector('.purchase-price').value));
                }
                productModal.hide();
            } else {
